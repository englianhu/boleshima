...
问黄历 <- function(阳历日期) {
  ...
  万年历 <- paste0('https://wannianrili.bmcx.com/', 阳历日期, '__wannianrili/')
  
  llply(1:length(万年历), function(迭) {
    
    年份 <- str_split(阳历日期, '-')[[迭]][1] %>% 
      str_replace_all('0', '零') %>% 
      str_replace_all('1', '一') %>% 
      str_replace_all('2', '二') %>% 
      str_replace_all('3', '三') %>% 
    ...
    农历月份 <- 黄历甲 |> 
      ...
      黄道吉日 <- c(
        黄历日期, 黄道吉日[3], 黄道吉日[5], 年份, 农历月份, 干支年, 
        黄道吉日[6], 宜[1], '宜', 宜[2], 黄道吉日[
          -c(1:7, grep('忌', 黄道吉日):length(黄道吉日))], '忌', 忌, 
        黄道吉日[c((grep('忌', 黄道吉日) + 1):length(黄道吉日))])
      ...
      rm(天干, 地支, 万年历, 年份, 黄历甲, 黄历乙, 黄历日期, 黄道吉日甲, 
         黄道吉日乙, 干支年, 阳历日期, 农历月份, 宜, 忌)
      
      藏经阁 <- '~/文档/猫城/zhongkehongqi-cangku/诸子百家学府/黄历书库/'
      if(!dir.exists(藏经阁)) dir.create(藏经阁)
      fwrite(黄道吉日, paste0(藏经阁, 黄道吉日, '.csv'))
      cat(paste0(藏经阁, 黄道吉日, '.csv 已储存！\n'))
