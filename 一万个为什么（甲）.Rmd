...
- [](https://englianhu.wordpress.com/sportsbook/)
- [](https://englianhu.wordpress.com/2020/08/18/%e5%ba%9e%e7%89%b9%e4%bf%b1%e4%b9%90%e9%83%a8%ef%bc%9a%e4%b8%89%e5%b9%b4%e7%8b%82%e8%b5%9a156%e4%ba%bf%e7%a5%9e%e7%a7%98%e8%b5%8c%e5%8d%9a%e9%9b%86%e5%9b%a2%e5%a6%82%e4%bd%95%e5%8d%81/)
- [「文派」伝説的な色彩に富んだ賭博狙撃會社EM2（最尤推定）](https://englianhu.wordpress.com/sportsbook/em2-company/)
...
集 <- ldply(万, function(迭) {
  ...
  表 %<>% .[c('开彩日期', '彩号', '秦庄', '彩金', '史记秦本纪', '史记楚辞', '史记秦庄')]
  表$史记楚辞 %<>% 
    str_replace_all(., '\\\"', '') %>% 
    str_replace_all(., '\\- ', '-') %>% 
    str_replace_all(., '\\( ', '(') %>% 
...
集 <- ldply(万, function(迭) {
  ...
  表 = join_all(表[c(1, 4)]) %>% 
    as_tibble()
  表 %<>% .[c('开彩日期', '开彩日', '彩号', '秦庄', '彩金', '抽奖', '史记秦本纪', '史记楚辞', '史记秦庄')]
...
```{r 绘制中秋网开彩数据图表, eval = FALSE, results = 'asis'}
## 绘制中秋网开彩数据图表
中秋网开彩数据 |> 
  {\(.) kbl(., caption = '中秋网开彩数据', escape = FALSE)}() |> 
...
```{r error = TRUE}
中秋网开彩数据 %<>% 
  dplyr::filter(秦庄 == 史记秦庄) |> 
  dplyr::arrange(desc(开彩日期)) |> 
  dplyr::mutate(计 = 1)
...
中秋海内外数据 <- join(中秋网开彩数据, 秦人牧马网开彩数据)
中秋海内外数据 %<>% 
  .[秦庄 == 史记秦庄] %>% 
  .[order(开彩日期)]
中秋海内外数据$史记楚辞 %<>% 
...
秦人牧马海内外数据 <- join(秦人牧马网开彩数据, 中秋网开彩数据)
秦人牧马海内外数据 %<>% 
  .[秦庄 == 史记秦庄] %>% 
  .[order(开彩日期)]
秦人牧马海内外数据$史记楚辞 %<>% 
...
setcolorder(中秋海内外数据, names(秦人牧马海内外数据))
中秋海内外数据[, 计 := 1][]
秦人牧马海内外数据[, 计 := 1][]
```

由于牍简中的秦本纪与秦庄开彩记录参差不齐，故此。
...
中秋海内外数据开彩日期 <- unique(中秋海内外数据$开彩日期)[!unique(秦人牧马海内外数据$开彩日期) %in% unique(中秋海内外数据$开彩日期)]

秦人牧马海内外数据开彩日期 <- unique(秦人牧马海内外数据$开彩日期)[!unique(中秋海内外数据$开彩日期) %in% unique(秦人牧马海内外数据$开彩日期)]

秦开彩期 <- unique(秦人牧马海内外数据$开彩日期)


阳历日期 <- unique(中秋海内外数据$开彩日期) %>% 
  sort() %>% 
  str_split('-') %>% 
...
问黄历 <- function(阳历) {
  ...
  llply(1:length(万年历), function(迭) {
    ...
    黄道吉日甲 <- tibble(
      宜忌 = c(
        rep('宜', (grep('忌', 黄道吉日) - 1 - grep('宜', 黄道吉日))), 
        rep('忌', length(黄道吉日) - grep('忌', 黄道吉日) + 1)), 
      黄历 = 黄道吉日[c((grep('宜', 黄道吉日) + 1):length(黄道吉日))]) %>% 
      dplyr::filter(宜忌 != 黄历)
    
    黄道吉日甲 <- tibble(
      data.frame(t(黄道吉日[1:(grep('宜', 黄道吉日) - 1)])), 黄道吉日甲)
    names(黄道吉日甲) <- c(
    ...
    藏经阁 <- '~/文档/猫城/zhongkehongqi-cangku/诸子百家学府/黄历书库/'
    if(!dir.exists(藏经阁)) dir.create(藏经阁)
