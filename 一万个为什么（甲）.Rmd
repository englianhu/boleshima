...
date: "🔆民国日期：民国一百一十三年菊月廿八<br>☯道家节日：[农历二零二四龙年菊月廿八 甲辰年（龙年）甲戌月丁卯日（愚生生日）](https://mobile.51wnl-cq.com/huangli_tab_h5/?posId=BDSS&STIME=2024-10-30)<br>农历生日吃寿面和红鸡蛋"
output:
  html_document: 
...
愚生旧东家乃英国百年老店福布斯五百强之一[立博](https://sports.ladbrokes.com)，英国（此称之为“楚国”，借鉴春秋战国时期楚国历史，包括所有共运会诸侯英殖民地）立博投注站遍布全国。借鉴愚生旧作，英国<ruby>蹴<rp>(</rp><rt>cù</rt><rp>)</rp>鞠<rp>(</rp><rt>jū</rt><rp>)</rp></ruby>赛事与投注开彩日一般上都是周日、周末、周三等、间中周二还会有开彩特别期，有个时间序列规律。
...
[泛马大彩有限公司推出更新版即刮即赢游戏](https://www.woah.my/2024/07/31/fan-ma-da-cai-you-xian-gong)此论文使用计数方式通过易经（包括广经堂、万年历）与计数编程，筹算马来西亚彩券开彩概率。

- [万能](https://www.magnum4d.my/zh-cn)：「钱并非万能，但没钱却万万不能。」
- [大马彩](https://www.damacai.com.my)：「田忌赛马，秦人牧马。」
...
它日再继续科研中国彩票吧。由于`forecast`、`fable`系列程序包具有完善的筹算、占卜、回测、检测配套，鉴于[Getting an error "`xreg` and `newxreg` have different numbers of columns"](https://stackoverflow.com/a/37642448/3806250)亦建议用之。
...
```{r 採撷中秋数据, eval = FALSE, error = TRUE}
## 商鞅变法与烧饼歌之「月到中秋分外明」
...
集 <- ldply(万, function(迭) {
  ...
  表 %<>% .[c('开彩日期', '彩号', '秦庄', '英庄', '奖', '秦典', '英典')]
  
  if(!exist('./诸子百家学府/4Dmoon')) dir.create('./诸子百家学府/4Dmoon')
  fwrite(表, paste0('./诸子百家学府/4Dmoon/马来西亚彩券数据_', 迭, '.csv'))
  }) %>% 
  as_tibble()
```

```{r 採撷秦统天下数据, eval = FALSE, error = TRUE}
## 秦统天下之父
...
集 <- ldply(万, function(迭) {
  ...
  表乙$英庄 = 链 %>% 
    ...
    html_nodes('td > img') %>% 
    html_attr('src') %>% 
    str_replace_all('https://4dpanda.com/assets/images/lotto/logo_|.gif', '')
  表乙 %<>% mutate(彩号 = sprintf('%04d', 彩号))
...
## 检验诸子百家学府书库。
if(!exist('./诸子百家学府/4Dmoon')) dir.create('./诸子百家学府/4Dmoon')
if(!exist('./诸子百家学府/4dresult88')) dir.create('./诸子百家学府/4dresult88')
...
借鉴[「鄀客栈」Lottery 3D Analysis 彩票分析](https://rpubs.com/englianhu/lottery-3D-analysis)中提及赌庄在开彩概率上占有百分之枯二点七的优势，欲以凯利投注模式获利，估计得花上百万年时间。借鉴「对酒当歌，人生几何？譬如朝露，去日苦多。」，所以得学习计数编程统筹法（环境与系统工程学），也就是咱们道家和民间习俗所说的“风水学”。将周遭所闻所见来实践《万年历》与《广经堂》中彩票辞典的陈腔滥调。
